...
class ClusterTreeBuilder(TreeBuilder):
    ...
    def construct_tree(
        ...
        for layer in tqdm(range(self.num_layers), desc="Building tree layers"):
            new_level_nodes = {}
            ...
            print(f"🔄 Layer {layer + 1}: Clustering {len(node_list_current_layer)} nodes...")
            ...
            print(f"✅ Layer {layer + 1}: Created {len(clusters)} clusters")
            ...
            print(f"🔄 Layer {layer + 1}: Processing {len(clusters)} clusters...")
            if use_multithreading:
                with ThreadPoolExecutor() as executor:
                    for cluster in tqdm(clusters, desc=f"Processing clusters for layer {layer + 1}"):
                        executor.submit(
                            process_cluster,
                            cluster,
                            new_level_nodes,
