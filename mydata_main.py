...
def main():
    ...
    parser.add_argument("--method", type=str, required=True, choices=["naive_p", "standard", "cosine_only", "all"], help="Method type: 'naive_p', 'standard', 'cosine_only', or 'all'")
    ...
    # 실행할 방법들 설정
    methods = ["naive_p", "standard", "cosine_only"] if args.method == "all" else [args.method]
    ...
    for method in methods:
        ...
        # 각 persona에 대해 실행
        if method == "standard" and args.mode == "indexing":
            mydata.run(-1)
        elif method == "standard" and args.mode == "all":
            mydata.run(-1)
